...
"
        ...
        str = [str stringByAppendingString:@"\
            window._oktoMsgCache = []; \
            window.addEventListener('message', function(event) { \
                try { \
                    console.log('Received message:', event); \
                    if (event && event.data) { \
                        window.Unity.call(JSON.stringify(event.data)); \
                    } else { \
                        window._oktoMsgCache.push(event); \
                    } \
                } catch (e) { \
                    console.error('Error forwarding message to Unity:', e); \
                } \
            }); \
            \
        ...
        WKUserScript *script
            = [[WKUserScript alloc] initWithSource:str injectionTime:WKUserScriptInjectionTimeAtDocumentStart forMainFrameOnly:NO];
